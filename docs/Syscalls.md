...
|--------|-----------------|----------------------------------|-------------------|----------------------|--------------|--------------|------|------|---------------------------------------|
| 0      | klog            | Log a message to the kernel log  | char* msg         |                      |              |              |      |      |                                       |
| 1      | allocate_memory | Allocate memory                  | size_t size       |                      |              |              |      |      | void*  address (null if failed)       |
| 2      | free_memory     | Free memory                      | void* address     |                      |              |              |      |      |                                       |
| 3      | resource_create | Create a resource                | ResourceType type | char* name           | size_t flags |              |      |      | int success (1 = success, 0 = failed) |
| 4      | resource_open   | Open a resource                  | ResourceType type | char* name           | size_t flags |              |      |      | uint64_t handle (0 if failed)         |
| 5      | resource_close  | Close a resource                 | uint64_t handle   | size_t flags         |              |              |      |      |                                       |
| 6      | resource_write  | Write to a resource              | uint64_t handle   | void* buffer         | size_t size  | size_t flags |      |      | size_t bytes written (0 if failed)    |
| 7      | resource_read   | Read from a resource             | uint64_t handle   | void* buffer         | size_t size  | size_t flags |      |      | size_t bytes read (0 if failed)       |
| 8      | registry_create | Register a new resource registry | char* endpoint    | size_t resource_type |              |              |      |      |                                       |

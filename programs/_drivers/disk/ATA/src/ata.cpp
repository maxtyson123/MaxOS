 ...
 */
AdvancedTechnologyAttachment::AdvancedTechnologyAttachment(device_identification_t& device_id, HardwareCommunication::hardware_mapping_t& hmap)
: m_data_port(hmap.ranges[0].address_base),
  m_error_port(hmap.ranges[0].address_base + 1),
  m_sector_count_port(hmap.ranges[0].address_base + 2),
  m_LBA_low_port(hmap.ranges[0].address_base + 3),
  m_LBA_mid_port(hmap.ranges[0].address_base + 4),
  m_LBA_high_Port(hmap.ranges[0].address_base + 5),
  m_device_port(hmap.ranges[0].address_base + 6),
  m_command_port(hmap.ranges[0].address_base + 7),
  m_control_port(hmap.ranges[1].address_base),
  m_is_master(device_id.class_info.interface)
{
 ...
 */
void AdvancedTechnologyAttachment::write(uint32_t sector, uint8_t* data_buffer, size_t amount) {

	// Don't allow writing more than a sector
	if (sector > 0x0FFFFFFF || amount > m_bytes_per_sector)
		return;
	...
	for (size_t i = 0; i < m_bytes_per_sector; i += 2) {
		...
		uint16_t write_data = data_buffer[i];

		// Place the next byte in the array if there is one
		if (i + 1 < amount)
			write_data |= (uint16_t) data_buffer[i + 1] << 8;

		m_data_port.write(write_data);
	}

	// Write the remaining bytes as a full sector has to be written
	for (size_t i = amount + (amount % 2); i < m_bytes_per_sector; i += 2)
		m_data_port.write(0x0000);

	// Wait for the device to finish writing
	status = m_command_port.read();
	while ((status & 0x80) != 0 || (status & 0x08) != 0) {
		yield();
		status = m_command_port.read();

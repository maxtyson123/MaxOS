...
namespace syscore {
	namespace filesystem {
		...
		size_t file_size(uint64_t handle){
			return resource_read(handle, 0, 0, (size_t)FileFlags::READ_SIZE);
		}

		size_t file_offset(uint64_t handle){
			return resource_read(handle, 0, 0, (size_t)FileFlags::READ_OFFSET);
		}
		...
		void seek_file(uint64_t handle, size_t position, SeekType seek_type){
			...
			switch (seek_type) {
				...
				case SeekType::SET:
					resource_write(handle, 0, position, (size_t)FileFlags::WRITE_SEEK_SET);
					break;

				case SeekType::CURRENT:
					resource_write(handle, 0, position, (size_t)FileFlags::WRITE_SEEK_CUR);
					break;

				case SeekType::END:
					resource_write(handle, 0, position, (size_t)FileFlags::WRITE_SEEK_END);
					break;

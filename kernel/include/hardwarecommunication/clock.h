...
#ifndef MAXOS_HARDWARECOMMUNICATION_CLOCK_H
...
namespace MaxOS::hardwarecommunication {
	...
	class PIT : public InterruptHandler {
		...
		public:
			explicit PIT(AdvancedProgrammableInterruptController* apic);
			~PIT();
	...
	class Clock : public InterruptHandler {
		...
		public:
			explicit Clock(AdvancedProgrammableInterruptController* apic, uint16_t time_between_events = 10);
			~Clock();
			...
			void setup_apic_clock(LocalAPIC* local_apic) const;
...
#endif //MAXOS_HARDWARECOMMUNICATION_CLOCK_H

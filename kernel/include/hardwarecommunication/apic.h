...
namespace MaxOS::hardwarecommunication {
	...
	class IOAPIC {
		private:
			...
			void read_redirect(uint8_t index, RedirectionEntry* entry) const;
			void write_redirect(uint8_t index, RedirectionEntry* entry) const;
	...
	class AdvancedProgrammableInterruptController {
		...
		public:
			...
			[[nodiscard]] LocalAPIC* local_apic() const;
			[[nodiscard]] IOAPIC* io_apic() const;

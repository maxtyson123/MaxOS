...
namespace MaxOS::drivers::clock {
	...
	class PIT : public hardwarecommunication::InterruptHandler {
		...
		public:
			explicit PIT(hardwarecommunication::AdvancedProgrammableInterruptController* apic);
			~PIT();
	...
	class Clock : public Driver, public hardwarecommunication::InterruptHandler {
		...
		public:
			explicit Clock(hardwarecommunication::AdvancedProgrammableInterruptController* apic, uint16_t time_between_events = 10);
			~Clock();
